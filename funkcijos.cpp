...
void skaitymas(std::vector<Studentas>& studentai, int irasukiekis)
{
...
// Funkcijos skirtos testavimui.

double vidurkioskaiciavimas(std::vector<double> testnd)
{
    double testvidurkis = 0;
    for (int j = 0; j < testnd.size(); j++) {
    std::cout << testvidurkis;
        testvidurkis += testnd.at(j);
    }

    testvidurkis = testvidurkis / testnd.size();
    return testvidurkis;
    
}
double medianosskaiciavimas(std::vector<double> testnd)
{
    double testmediana = 0;
    int pozicija = testnd.size();
    sort(testnd.begin(), testnd.end());


    if (pozicija % 2 == 0)
    {
        testmediana = ((testnd.at(pozicija / 2)) + (testnd.at(pozicija / 2 - 1))) / 2;
    }
    else {
        testmediana = testnd.at(pozicija / 2);
    }
    return testmediana;
}